...
count(employee_id) as Numero_Empleados,
...
count(e.employee_id) as 'Número de Empleados'
from employee e
...
count(e.employee_id) as numEmpleados,
...
max(salary) as Salario_Máximo,
min(salary) as Salario_Mínimo
from employee e
JOIN department d on e.department_id = d.department_id
WHERE e.department_id IS NOT NULL
GROUP BY department_name
HAVING COUNT(employee_id) >= 5 AND Salario_Promedio > 5000;

#3. Obtener el apellido, nombre de departamento y nombre de país de los empleados que son Manager.
SELECT e.employee_id as Código_Empleado,
